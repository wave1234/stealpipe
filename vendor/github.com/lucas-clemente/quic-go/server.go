...
func (s *server) serve() {
	...
	for {
		...
		n2 := conv.Decoder(data, n)
		n = n2

		data = data[:n]
		//fmt.Println("Server ReadFrom ",n)
...
func (s *server) handlePacket(pconn net.PacketConn, remoteAddr net.Addr, packet []byte) error {
	...
	conv.Init(s.config)
       ...
       tmpconn = &conn{pconn: pconn, conv: conv, currentAddr: remoteAddr}
    if !sessionKnown && (!hdr.VersionFlag && hdr.Type != protocol.PacketTypeInitial) {
		fmt.Println("pconn,Write1")

        //_, err = pconn.WriteTo(wire.WritePublicReset(connID, 0, 0), remoteAddr)
        err = tmpconn.Write(wire.WritePublicReset(connID, 0, 0))
		return err
	}
	...
	if hdr.VersionFlag && !protocol.IsSupportedVersion(s.config.Versions, hdr.Version) {
		...
		fmt.Println("pconn, write2")

        //if _, err := pconn.WriteTo(wire.ComposeGQUICVersionNegotiation(hdr.ConnectionID, s.config.Versions), remoteAddr); err != nil {
        if err := tmpconn.Write(wire.ComposeGQUICVersionNegotiation(hdr.ConnectionID, s.config.Versions)); err != nil {
			return err
		}
	}
	...
	if !sessionKnown {
		...
		session, err = s.newSession(
			&conn{pconn: pconn, conv: conv, currentAddr: remoteAddr},
			version,
			hdr.ConnectionID,
			s.scfg,
			s.tlsConf,
